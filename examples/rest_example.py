...
from gladiapy.v2.rest_models import TranscriptionRequest, ListResultsQuery
...
def main():
    ...
    try:
        ...
        while status not in ("done", "error"):
            ...
            time.sleep(3)
        # Get final result
        final_result = client.get_result(job.id)
        if final_result.status == "done" and final_result.result and final_result.result.transcription:
            ...
            print(f"Transcript: \"{trans.full_transcript}\"")
        ...
        try:
            page = client.get_results(ListResultsQuery(offset=0, limit=5))
